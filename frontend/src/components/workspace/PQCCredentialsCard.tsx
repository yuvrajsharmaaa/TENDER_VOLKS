...
export const PQCCredentialsCard: React.FC<PQCCredentialsCardProps> = ({
  ...
  const credentialsToDisplay: MatchedCredential[] = data.qualifies 
    ? data.matched_credentials.slice(0, 3)
    : (data.closest_candidates || []).slice(0, 3);

  const isQualified = data.qualifies || data.qualification_status === "QUALIFIED";
    ...
    <div className={`bg-card-bg border border-divider rounded-xl p-4 shadow-xs select-none ${className}`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pb-3 mb-3 border-b border-divider">
        ...
        <div className="flex items-center gap-2">
          {/* Qualification Badge */}
          <div
            className={`px-2.5 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 border shadow-2xs ${
              isQualified
          ...
          >
            ...
            <span>
              {isQualified ? "QUALIFIED" : "DISQUALIFIED"} • {data.strategy_used || "NO_MATCH"}
            </span>
          </div>
